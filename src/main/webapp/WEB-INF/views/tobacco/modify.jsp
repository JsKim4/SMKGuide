			...
			<div class="panel-body">
				<form role="form" action="./modify" method="post"
					...
					<div class="col-lg-9">
						...
						<div class="form-group">
							<label>quantity</label> <input class="form-control"
								name='quantity' value='<c:out value="${tobacco.quantity }"/>'>
						</div>
						<div class="form-group">
					...
					<div class="form-group col-lg-3">
						<label>brand</label> <select name='brand.id'>
							<c:forEach items="${brandList}" var="brand">
								<option value="${brand.id}"
									<c:out value="${tobacco.brand.id==brand.id?'selected':''}"/>>${brand.name }</option>
							</c:forEach>
						</select>
					</div>
					<div class="form-group col-lg-3">
						<label>country</label> <select name='country.id'>
							<c:forEach items="${countryList}" var="country">
								<option value="${country.id}"
									<c:out value="${tobacco.country.id==country.id?'selected':''}"/>>${country.name }</option>
							</c:forEach>
						</select>
					</div>
					<div class="form-group col-lg-3">
						<label>company</label> <select name='company.id'>
							<c:forEach items="${companyList}" var="company">
								<option value="${company.id}"
									<c:out value="${tobacco.company.id==company.id?'selected':''}"/>>${company.name }</option>
							</c:forEach>
						</select>
					</div>
					<div class="form-group col-lg-3">
						<label>type</label> <select name='type.id'>
							<c:forEach items="${typeList}" var="type">
								<option value="${type.id}"
									<c:out value="${tobacco.type.id==type.id?'selected':''}"/>>${type.name }</option>
							</c:forEach>
						</select>
					</div>
					<div class='form-group'>
						<input type="file" name="uploadFile"
							value="${tobacco.attach.fileName}">
					</div>
					<div class="col-lg-12">
