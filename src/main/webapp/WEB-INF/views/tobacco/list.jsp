				...
				<div class='pull-right'>
					<ul class="pagination">
						<c:if test="${pageMaker.prev}">
							<li class="paginate_button previous"><a
								href="${pageMaker.startPage-1 }"> <c:out value="Previous" />
							</a></li>
						</c:if>
						<c:forEach var="num" begin="${pageMaker.startPage}"
							end="${pageMaker.endPage}">
							<li class="paginate_button ${pageMaker.cri.pageNum == num ? "active" : "" }"><a
								href="${num}"> <c:out value="${num}" />
							</a></li>
						</c:forEach>
						<c:if test="${pageMaker.next}">
							<li class="paginate_button next"><a
								href="${pageMaker.endPage+1 }"> <c:out value="Next" />
							</a></li>
						</c:if>
					</ul>
					<form id='actionForm' action="./list" method='get'>
						...
						<input type='hidden' name='keyword'
							...
							name='tId' value='${pageMaker.cri.TId}'> <input
							type='hidden' name='mId' value='${pageMaker.cri.MId}'> <input
							type='hidden' name='nId' value='${pageMaker.cri.NId}'> <input
							type='hidden' name='bId' value='${pageMaker.cri.BId}'>
					</form>
					<form id='searchForm' action="./list" method='get'>
						<div>
							<select name='bId'>
								<option value=""
									<c:out value="${pageMaker.cri.BId==null?'selected':''}"/>>Brand</option>
								<c:forEach items="${brandList}" var="brand">
									<option value="${brand.id}"
										<c:out value="${pageMaker.cri.BId==brand.id?'selected':''}"/>>${brand.name }</option>
								</c:forEach>
							</select> <select name='tId'>
								<option value="">Type</option>
								<c:forEach items="${typeList}" var="type">
									<option value="${type.id}"
										<c:out value="${pageMaker.cri.TId==type.id?'selected':''}"/>>${type.name }</option>
								</c:forEach>
							</select> <select name='nId'>
								<option value="">country</option>
								<c:forEach items="${countryList}" var="country">
									<option value="${country.id}"
										<c:out value="${pageMaker.cri.NId==country.id?'selected':''}"/>>${country.name }</option>
								</c:forEach>
							</select> <select name='mId'>
								<option value="">Company</option>
								<c:forEach items="${companyList}" var="company">
									<option value="${company.id}"
										<c:out value="${pageMaker.cri.MId==company.id?'selected':''}"/>>${company.name }</option>
								</c:forEach>
			...
			.ready(
					function() {
						...
						function checkModal(result) {
							...
							if (result === 'success')
								$(".modal-body").html("수정에 성공하였습니다.");
							else if (parseInt(result) > 0) {
								$(".modal-body").html(
										"담배 " + parseInt(result)
												+ " 번이 등록되었습니다.");
							} else {
								$(".modal-body").html("등록이 실패하였습니다.");
